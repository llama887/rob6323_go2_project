...
class Rob6323Go2Env(DirectRLEnv):
    ...
    def __init__(self, cfg: Rob6323Go2EnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # Action history buffer for rate/accel penalties (current + 2 previous)
        self.last_actions = torch.zeros(self.num_envs, action_dim, 3, device=self.device, requires_grad=False)
        ...
        # Feet indices for kinematics and contact sensing
        self._feet_ids = []
    ...
    def foot_positions_w(self) -> torch.Tensor:
        """Return feet positions in world frame. Shape: (num_envs, 4, 3)."""
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # Action rate penalty (first + second derivative)
        ...
        # roll history buffer and insert current action
        self.last_actions = torch.roll(self.last_actions, 1, 2)
        self.last_actions[:, :, 0] = self._actions[:]

        # Gait & Raibert heuristic (clock already advanced in _get_observations)
        rew_raibert_heuristic = self._reward_raibert_heuristic()

        # Posture / smoothness penalties
        ...
        # Foot clearance during swing (penalize low swing height relative to a nominal clearance)
        ...
        rew_feet_clearance = torch.sum(
            torch.square(torch.clamp(clearance_target - foot_heights, min=0.0)) * swing_mask, dim=1
        )

        # Contact force tracking (match desired contact probability produced by gait planner)
        contact_forces = self._contact_sensor.data.net_forces_w[:, self._feet_ids_sensor]
        contact_force_mag = torch.linalg.norm(contact_forces, dim=-1)
        contact_prob = torch.tanh(contact_force_mag / 40.0)
        rew_tracking_contacts = torch.sum(torch.square(self.desired_contact_states - contact_prob), dim=1) * -1.0
    ...
    def _step_contact_targets(self):
        """Advance gait phase and compute desired contact states + clock inputs."""
        frequencies = 3.0
        phases = 0.5
        offsets = 0.0
        bounds = 0.0
        ...
        # remap stance/swing to [0, 0.5] and [0.5,1] with smoothing
        ...
        # Smooth desired contact states using a normal CDF window
        kappa = 0.07
        smoothing_cdf = torch.distributions.normal.Normal(0, kappa).cdf
        def _smooth(idxs):
            return (
                smoothing_cdf(torch.remainder(idxs, 1.0))
                * (1 - smoothing_cdf(torch.remainder(idxs, 1.0) - 0.5))
                + smoothing_cdf(torch.remainder(idxs, 1.0) - 1)
                * (1 - smoothing_cdf(torch.remainder(idxs, 1.0) - 0.5 - 1))
            )

        self.desired_contact_states[:, 0] = _smooth(foot_indices[0])
        self.desired_contact_states[:, 1] = _smooth(foot_indices[1])
        self.desired_contact_states[:, 2] = _smooth(foot_indices[2])
        self.desired_contact_states[:, 3] = _smooth(foot_indices[3])

    def _reward_raibert_heuristic(self):
        """Squared error between desired and actual footsteps per Raibert heuristic."""
